...
def _get_best_unique_element(
    ...
    This function is similar of `np.unique(return_index=True)` and behaves in
    ...
    for i, i1 in enumerate(unique_weights):
        ...
        # that weight[index_when_sorted] is the maximum weight for value
        ...
        index_when_sorted = i1 + np.argmax(weights[i1:i2])
...
def _find_slice(original: np.ndarray, sliced: np.ndarray) -> tuple[int, int]:
    ...
    `i` and `j` that have been used to obtain the slice `w` from the original
    array `v`.
    ...
    # element for each value of slice
...
def read_efas_data_files(
    ...
    The function reads the different kind of files, merges them,
    ...
    Args:
        ...
        config_content: The content of the csv config file
    ...
    """
    # This function is just a wrapper that decompress the (optionally)
...
def _read_efas_forecast_file(dataset: xr.Dataset):
    ...
    # i.e. for the same time of the model we have the forecast produced 6 hours
    # before, 18 hours before, and so on... We must remove this redundancy by
    # selecting only the most reliable forecast (the one that is closest to the
    # computation date).
...
def _read_unzipped_efas_files(
    ...
    Args:
        input_files (Iterable[Path]): A collection of file paths indicating the
            locations of the unzipped EFAS files to process.
        config_content (pd.DataFrame): A DataFrame containing configuration
            information for EFAS rivers, including indices of river locations.
        efas_domain_file (PathLike): A path to the EFAS domain file used for
            latitude and longitude referencing.
    ...
    # Get the ids of the rivers
    rivers_id = np.array(
        config_content.index.get_level_values("id"), dtype=int
    )

    # Extract data at EFAS river
    lon_indices = xr.DataArray(
        config_content["mouth_longitude_index"],
        dims=["id"],
        coords={"id": rivers_id},
    )
    lat_indices = xr.DataArray(
        config_content["mouth_latitude_index"],
        dims=["id"],
        coords={"id": rivers_id},
    )
        ...
        with xr.open_dataset(file_path) as f:
            ...
            def select_points(single_ds: xr.Dataset):
                ...
                if np.any(outside_lat) or np.any(outside_lon):
                    ...
                    river_name = config_content["name"][river_outside_index]
            ...
            # named discharge and the two dimensions are the index of the river
            # ("id") and the time step ("time")
...
def generate_efas_climatology(
    ...
    with ExitStack() as cleanup_operations:
        ...
        # years to the respective files. If this is the case, we check which
